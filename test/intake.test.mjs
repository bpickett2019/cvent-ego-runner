...
test('RR and explicit target upload gets fixed instructions; read starts without browser permission', async t => {
  ...
  assert.match(f.job.instruction, /Do not delete/);
  ...
  assert.match(prompts(f.launches[0])[0].message, /Reuse existing items unchanged/);
...
test('genuine clarification then automatic setup uses one live session, no restoration', async t => {
  ...
  assert.match(prompts(rpc).at(-1).message, /Do not reread or dump/);
  assert.match(prompts(rpc).at(-1).message, /Reuse existing items unchanged/);
  assert.match(prompts(rpc).at(-1).message, /Never change the event name/);
  assert.match(await readFile(join(f.job.workspace, 'approved-sow.md'), 'utf8'), /Reuse existing items unchanged/);
  rpc.emit('event', { type: 'agent_settled' });
  await f.wait(d => d.status === 'REVIEW_REQUIRED');
